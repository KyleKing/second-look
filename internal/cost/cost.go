...
// Of fetches a pull request's diff and rates it. It is one API read per call,
// so a caller rating a queue bounds how many run at once.
func Of(ctx context.Context, dir, repository string, number int) (rate.Score, error) {
	patch, err := github.PRDiff(ctx, dir, repository, number)
	if err != nil {
		return rate.Score{}, fmt.Errorf("reading the diff of %s#%d: %w", repository, number, err)
	}

	readings, _, err := rate.Read(ctx, diff.Parse(patch))
	if err != nil {
		return rate.Score{}, fmt.Errorf("rating %s#%d: %w", repository, number, err)
	}

	return rate.Of(readings), nil
}
